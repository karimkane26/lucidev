  ...
  return (
    <section id="contact" className="section">
      <div className="container">
        <div className="bg-white dark:bg-zinc-800/60 border border-zinc-200/80 dark:border-zinc-700/60 rounded-3xl p-8 md:p-12 shadow-sm reveal-up">
          <div className="lg:grid lg:grid-cols-2 lg:gap-16 lg:items-start">

            {/* ── Left ── */}
            <div className="mb-12 lg:mb-0">
              <span className="section-label">Contact</span>
              <h2 className="headline-2 lg:max-w-[14ch] mb-4">
                Travaillons ensemble
              </h2>
              <p className="text-zinc-500 dark:text-zinc-400 max-w-[38ch] leading-relaxed mb-8">
                Vous avez un projet en tête ? Contactez-moi pour en discuter et démarrer quelque chose d'exceptionnel.
              </p>

              {/* Contact info */}
              <div className="space-y-3 mb-10">
                <a href="mailto:lucidev97@gmail.com" className="flex items-center gap-3 text-sm text-zinc-600 dark:text-zinc-300 hover:text-sky-500 dark:hover:text-sky-400 transition-colors group">
                  <span className="w-9 h-9 rounded-xl bg-zinc-100 dark:bg-zinc-700/60 grid place-items-center shrink-0 group-hover:bg-sky-50 dark:group-hover:bg-sky-500/10 transition-colors">
                    <span className="material-symbols-rounded text-[17px] text-zinc-500 group-hover:text-sky-500 transition-colors">mail</span>
                  </span>
                  lucidev97@gmail.com
                </a>
                <div className="flex items-center gap-3 text-sm text-zinc-600 dark:text-zinc-300">
                  <span className="w-9 h-9 rounded-xl bg-zinc-100 dark:bg-zinc-700/60 grid place-items-center shrink-0">
                    <span className="material-symbols-rounded text-[17px] text-zinc-500">location_on</span>
                  </span>
                  Dakar, Sénégal
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                {socialLinks.map(({ href, icon, label }) => (
                  <a
                    ...
                    rel="noreferrer"
                    aria-label={label}
                    className="w-10 h-10 grid place-items-center rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 hover:bg-zinc-900 hover:text-white hover:border-zinc-900 dark:hover:bg-sky-500 dark:hover:border-sky-500 dark:hover:text-white transition-all active:scale-95"
                  >
                    {icon}
                  </a>
                ))}
            ...
            {/* ── Form ── */}
            <form action="https://getform.io/f/apjjewwa" method="POST" className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  required
                  className="text-field resize-y min-h-36 max-h-72"
                ></textarea>
              </div>

              <button
                type="submit"
                className="btn btn-primary w-full justify-center [&]:max-w-full"
              >
                Envoyer le message
